...
class Territory(object.GameObject):
    def __init__(self, location, radius, mooseNumber):
        ...
        for n in range(mooseNumber):
            ...
            if n > 0:
                self.population.append(moose.Moose(self.population[0].location - xOffset, random.uniform(settings.MIN_MOOSE_LENGTH, settings.MAX_MOOSE_LENGTH) * settings.PIXELS_PER_METER, self, self.population[n-1]))
            else:
                self.population.append(moose.Moose(self.location + randDir, random.uniform(2.5, 3) * settings.PIXELS_PER_METER, self))
